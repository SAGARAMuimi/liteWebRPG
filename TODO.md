...
#### R-20 ステータス表示改善（最小実装）
- [X] `pages/3_battle.py` のパーティカードに `ATK {chara.attack}  DEF {chara.defense}` を追加
- [X] `pages/2_dungeon.py` のパーティ欄にも同様に ATK/DEF を追加
- **依存**: なし（独立実装可）
...
#### R-23 ダンジョン入場前チェック
- [x] `config.py` に各ダンジョンの推奨レベル定数を追加
- [x] `pages/2_dungeon.py` の `render_dungeon_select()` に推奨レベル vs パーティ平均 Lv の判定ロジックを追加
- [x] 判定結果（✅ / ⚠️ / 🔴）をダンジョンカードに1行表示
- **依存**: なし（独立実装可）

#### R-28 バトルスピード設定
- [x] `models/user.py` に `battle_speed VARCHAR(8) DEFAULT 'normal'` カラムを追加
- [x] `models/database.py` の `migrate_db()` に `battle_speed` カラム追加マイグレーションを追加
- [x] `pages/1_character.py` または `app.py` のサイドバーに `st.radio` で速度選択 UI を追加・DB 保存
- [x] `pages/3_battle.py` の `time.sleep()` 呼び出しをセッションの設定値から取得するよう変更
- **依存**: なし（独立実装可）
...
#### R-31 レベルアップ習得スキル
- [ ] `models/skill.py` に `required_level INTEGER NOT NULL DEFAULT 1` カラムを追加
- [ ] `models/database.py` の `migrate_db()` に `required_level` カラム追加マイグレーションを追加
- [ ] `utils/helpers.py` または seed 処理で各スキルの `required_level` を設定（Lv1基本スキル / Lv3・Lv5上位スキル など）
- [ ] `models/skill.py` に `Skill.get_for_character(db, character)` 相当のレベル条件付き取得関数を追加
- [ ] `pages/3_battle.py` のスキル表示を `class_type + level` 条件に変更し、未習得スキルは戦闘中に表示しない
- [ ] `game/battle.py` の味方 AI でも同じレベル条件付きスキル一覧を参照するよう統一
- [ ] `pages/1_character.py` に「習得済み / Lv○で習得予定」の一覧表示を追加（任意UIを含む）
- [ ] レベルアップ直後に新規解放スキルがある場合、`st.success()` で習得通知を表示
- [ ] **既存キャラ対応**: `character_skills` のような個別保存は導入せず、現在レベルによる動的判定で解放するためバックフィル不要とする
- **依存**: R-21（スキル説明）と同時に `skills` テーブル変更をまとめると効率的
...
| 発見日 | 状態 | 概要 |
|---|---|---|
| 2026-03-29 | ✅ 修正済み (2026-03-30) | Neon Auth ログイン直後のページ遷移で「セッションが期限切れです」 |

### BUG-01: Neon Auth ログイン直後ページ遷移でセッション切れ扱い

**現象:**  
`local` バックエンドは問題なし。`neon` バックエンドでログイン成功後、別ページ（例: `1_character.py`）に遷移すると
「セッションが期限切れです。再ログインしてください。」が表示されてログイン画面に戻される。

**根本原因:**  
`login_user()` で Neon ログイン成功時に `neon_token_validated_at` をセッションにセットしていない。  
→ 次ページの `check_login()` → `_neon_validate_session_if_needed()` が `last_validated = 0.0` と判定し、
再検証インターバル（300秒）を超えているとみなして JWKS / `/get-session` で再検証を試みる。  
→ この検証が何らかの理由で失敗するとセッションを全クリアして `False` を返す。

**修正箇所:** `utils/auth.py` の `login_user()` 内、neon ログイン成功時の末尾
...
st.session_state["neon_token_validated_at"] = time.time()  # ← 追加
return True
```

**同様に** `register_user()` の neon ブランチでトークンを発行してセッションをセットする箇所にも同じ1行を追加すること。
